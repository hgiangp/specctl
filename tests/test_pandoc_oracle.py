...
def test_an_independent_reader_can_open_every_built_stand_in(name: str) -> None:
    ...
    that happens to satisfy our own parser — lxml reads anything well-formed, so it cannot
    tell the difference. Pandoc can: it implements the schema, and it is the reader that
    exposed the tracked-changes shape as wrong.
    ...
    w = D.NS["w"]
    in_a_text_box = {
        normalize("".join(t.text or "" for t in box.iter(f"{{{w}}}t")))
        for box in D.parse_part(path).iter(f"{{{w}}}txbxContent")
    }
    haystack = normalize(result.text)
    lost = [
        segment
        for segment in source_segments(path)
        if segment not in in_a_text_box and not covered(segment, haystack)
    ]
    assert not lost, (
